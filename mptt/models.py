    ...
    def update_mptt_cached_fields(self, instance):
        ...
        for field_name in field_names:
            if deferred_fields:
                field = instance._meta.get_field(field_name)
                if (
                    field.attname in deferred_fields
...
class MPTTModel(models.Model, metaclass=MPTTModelBase):
    ...
    def _mptt_track_tree_insertions(cls, tree_id, num_inserted):
        ...
        new_changes = {(t + num_inserted if t >= tree_id else t) for t in changes}
    ...
    def get_descendants(self, include_self=False):
        ...
        return self._tree_manager._mptt_filter(
            tree_id=self._mpttfield("tree_id"), left__gte=left, left__lte=right
        )
    ...
    def get_leafnodes(self, include_self=False):
        ...
        return self._tree_manager._mptt_filter(
            descendants, left=(models.F(self._mptt_meta.right_attr) - 1)
        )
    ...
    def get_root(self):
        ...
        """
        if self.is_root_node() and type(self) is self._tree_manager.tree_model:
            return self
    ...
    def insert_at(
        ...
        _tm.insert_node(
            ...
            position,
            save,
            allow_existing_pk=allow_existing_pk,
